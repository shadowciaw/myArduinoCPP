...
int readUnsigned32()
{
    ...
    */
    ...
    char str[32];        // creates array of length 16 named str
    readString(str, 32); // calls readString() function
...
void server()
{
    ...
    while (true)
    {
        ...
        // wait until client has sent public key
...
void client(int ckey)
{
    ...
    while (true)
    {
        ...
        if (wait_on_serial3(5, 1000))
        {
            ...
            uint8_t read = Serial3.read();
            if ((char)read == 'A')
            {
                // if received message is ACK(skey);
                // 'A' followed by 4 bytes of shared key
                break;
            }
        }
    }
...
uint8_t encr(uint8_t byte, uint32_t key, uint rounds)
{
    /*
